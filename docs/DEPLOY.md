...
- `OPS_DATABASE_URL` — optional, and only while the deployment is still yours
  alone. It is a psql DSN for the `ctb_ops` role (`scripts/ops_role.sql`): read
  and write across every tenant, `BYPASSRLS`, no superuser. It exists so a
  stuck row can be looked at and repaired from a terminal. Nothing in the bot
  reads it either. Once somebody else's data is in the database, narrowing or
  dropping this role is the first thing to do.
